...
export const BookingFlowView: React.FC = () => {
  ...
  if (step === 'confirmed') {
    return (
      <div className="p-6 text-center space-y-6 pt-10">
        ...
        <div>
          ...
          <h2 className="text-xl font-bold text-[#18181B] mt-2">Your Booking is Scheduled!</h2>
          <p className="text-xs text-[#6B7280] mt-1">
            Reference Number: <span className="font-mono font-bold text-[#18181B]">{confirmedBookingRef}</span>
          </p>
        </div>
        ...
        <Card className="p-4 space-y-3 text-left bg-white">
          <div className="flex items-center justify-between border-b border-[#E5E7EB] pb-3">
            <div>
              <div className="text-xs font-bold text-[#18181B]">{service.name}</div>
              <div className="text-[11px] text-[#6B7280]">
                For {activeVehicle.make} {activeVehicle.model} ({activeVehicle.registrationNumber})
              </div>
            </div>
            <span className="font-bold text-sm text-[#18181B]">₹{totalPayable}</span>
          </div>

          <div className="space-y-1.5 text-xs text-[#6B7280]">
            <div className="flex items-center gap-2">
              <Calendar className="w-4 h-4 text-[#2457FF]" />
              <span>{selectedDate} at {selectedSlot}</span>
            </div>
            <div className="flex items-center gap-2">
              <MapPin className="w-4 h-4 text-[#2457FF]" />
              <span>{activeAddress.line1}, {activeAddress.city}</span>
            </div>
          </div>
        </Card>

        <div className="bg-[#EEF2FF] p-4 rounded-2xl text-left border border-[#2457FF]/20 space-y-1">
          <div className="text-xs font-bold text-[#2457FF] flex items-center gap-2">
            <span className="w-2 h-2 rounded-full bg-[#2457FF] animate-ping" />
  ...
  return (
    <div className="p-4 space-y-5">
      ...
      <div className="flex items-center justify-between border-b border-[#E5E7EB] pb-3">
        ...
        <span className="text-xs text-[#6B7280] font-mono">Step {step === 'slot' ? '1/3' : step === 'summary' ? '2/3' : '3/3'}</span>
      </div>
                ...
                <button
                  ...
                  className={`p-3 rounded-2xl border text-center transition-all ${
                    selectedDate === opt.date
                      ? 'border-[#2457FF] bg-[#2457FF] text-white shadow-xs'
                      : 'border-[#E5E7EB] bg-white text-[#18181B] hover:border-[#D1D5DB]'
                  }`}
                >
              ...
              {timeSlots.map((slot) => (
                <div
                  ...
                  className={`p-3 rounded-xl border flex items-center justify-between cursor-pointer transition-all ${
                    selectedSlot === slot
                      ? 'border-[#2457FF] bg-[#EEF2FF]/50 text-[#2457FF] font-semibold'
                      : 'border-[#E5E7EB] bg-white text-[#18181B] hover:border-[#D1D5DB]'
                  }`}
                >
                  <div className="flex items-center gap-2.5 text-xs">
                    <Clock className="w-4 h-4 text-[#2457FF]" />
                    <span>{slot}</span>
                  </div>
                  {selectedSlot === slot && <Badge variant="accent">Selected</Badge>}
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="space-y-4">
          <Card className="space-y-3">
            ...
            <div className="text-xs space-y-1">
              <div className="font-bold text-[#18181B] text-sm">{service.name}</div>
              <div className="text-[#6B7280]">Vehicle: {activeVehicle.make} {activeVehicle.model} ({activeVehicle.registrationNumber})</div>
              <div className="text-[#6B7280]">Slot: {selectedDate} • {selectedSlot}</div>
              <div className="text-[#6B7280]">Address: {activeAddress.line1}, {activeAddress.city}</div>
            </div>
          </Card>
          ...
          {stateAddons.length > 0 && (
            <Card className="space-y-2">
              <div className="text-xs font-bold text-[#18181B]">Selected Add-ons</div>
              {stateAddons.map((addon) => (
                <div key={addon.id} className="flex justify-between text-xs text-[#6B7280]">
                  <span>{addon.name}</span>
                  <span className="font-semibold text-[#18181B]">₹{addon.price}</span>
                </div>
              ))}
            </Card>
          )}
          ...
          {/* Coupon Code Field */}
          <Card className="space-y-2">
            <div className="text-xs font-bold text-[#18181B]">Apply Offer Code</div>
            <div className="flex gap-2">
              <Input
                placeholder="Enter code e.g. FIRSTWASH"
                value={couponCode}
            ...
            {appliedCoupon && (
              <div className="text-xs text-[#17A34A] font-semibold flex items-center gap-1 mt-1">
          ...
          {/* Price Breakdown */}
          <Card className="space-y-2 bg-[#F4F5F7]">
            ...
            {finalDiscount > 0 && (
              <div className="flex justify-between text-xs text-[#17A34A] font-semibold">
            ...
            <div className="flex justify-between text-sm font-bold text-[#18181B] pt-2 border-t border-[#E5E7EB]">
      ...
      {step === 'payment' && (
        <div className="space-y-4">
          <Card className="bg-[#18181B] text-white p-4">
            <div className="text-xs text-[#A1A1AA]">Amount to Pay</div>
            <div className="text-2xl font-bold text-white mt-0.5">₹{totalPayable}</div>
            <div className="text-[11px] text-[#A1A1AA] mt-2 flex items-center gap-1">
          ...
          <div className="space-y-2">
            ...
            <div
              ...
              className={`p-3 rounded-xl border flex items-center justify-between cursor-pointer ${
                paymentMethod === 'upi' ? 'border-[#2457FF] bg-[#EEF2FF]/50' : 'border-[#E5E7EB] bg-white'
              }`}
            >
              <div className="flex items-center gap-3">
                <div className="w-8 h-8 rounded-lg bg-blue-100 text-[#2457FF] flex items-center justify-center font-bold text-xs">
                  UPI
                </div>
                <div>
                  <div className="text-xs font-bold text-[#18181B]">UPI (Google Pay, PhonePe, Paytm)</div>
                  <div className="text-[11px] text-[#6B7280]">Instant 1-tap app redirect</div>
                </div>
              </div>
              {paymentMethod === 'upi' && <Badge variant="accent">Selected</Badge>}
            </div>

            <div
              onClick={() => setPaymentMethod('card')}
              className={`p-3 rounded-xl border flex items-center justify-between cursor-pointer ${
                paymentMethod === 'card' ? 'border-[#2457FF] bg-[#EEF2FF]/50' : 'border-[#E5E7EB] bg-white'
              }`}
            >
              <div className="flex items-center gap-3">
                <CreditCard className="w-8 h-8 p-1.5 rounded-lg bg-gray-100 text-gray-700" />
                <div>
                  <div className="text-xs font-bold text-[#18181B]">Credit / Debit Card</div>
                  <div className="text-[11px] text-[#6B7280]">Visa, Mastercard, RuPay</div>
                </div>
              </div>
              {paymentMethod === 'card' && <Badge variant="accent">Selected</Badge>}
            </div>
          </div>
